...
// A bounded cell queue with a pending bitmap. Entries may be candidate clue
// cells; the owner checks the state RAM after popping an entry.
module solver_dirty_fifo #(
    ...
    reg [8:0] head;
    reg [8:0] tail;
    reg [MAX_CELLS-1:0] pending_bitmap;
    wire [9:0] queue_head_data = queue_mem[head];

    wire push_in_range = push_index < MAX_CELLS;
    assign push_duplicate = push_in_range && pending_bitmap[push_index];
        ...
        if (reset) begin
            ...
            case ({push_valid && push_ready, pop_valid && pop_ready})
                2'b10: begin
                    ...
                    pending_bitmap[push_index] <= 1'b1;
                    tail <= increment_pointer(tail);
                    count <= count + 1'b1;
                end
                2'b01: begin
                    pending_bitmap[pop_index] <= 1'b0;
